...
class CCNetBackbone(torch.nn.Module):
    '''
    CompNet Backbone (Without Classification ArcMargin)
    Returns feature vectors.
    '''

    def __init__(self, in_channels=3, weight=0.8):
        super(CCNetBackbone, self).__init__()

        self.in_channels = in_channels
        self.weight = weight

        self.cb1 = CompetitiveBlock_Mul_Ord_Comp(channel_in=in_channels, n_competitor=9, ksize=35, stride=3, padding=17, init_ratio=1, weight=weight)
        self.cb2 = CompetitiveBlock_Mul_Ord_Comp(channel_in=in_channels, n_competitor=36, ksize=17, stride=3, padding=8, init_ratio=0.5, o2=24, weight=weight)
        self.cb3 = CompetitiveBlock_Mul_Ord_Comp(channel_in=in_channels, n_competitor=9, ksize=7, stride=3, padding=3, init_ratio=0.25, weight=weight)

        # Using LazyLinear to support arbitrary input image sizes without breaking
        self.fc = nn.LazyLinear(4096)
